...
const repairSql = "supabase/preprod_legacy_mirror_and_attendance_period_repair_2026-05-23.sql";
...
if (!shouldApply) {
  ...
  console.error("Expected usage: node scripts/preprod-legacy-mirror-repair-psql.mjs --db-url-file <path> --apply-auto-repair");
  process.exit(2);
}
...
if (!allowKnownProd && knownProdProjectRefs.some((projectRef) => rawDbUrl.includes(projectRef))) {
  console.error("Refusing to run legacy mirror repair against the known production project ref.");
...
const absoluteFile = resolve(repairSql);
if (!existsSync(absoluteFile)) {
  console.error(`Missing SQL: ${repairSql}`);
  process.exit(2);
}

console.log(`\n## apply deterministic legacy mirror repair: ${repairSql}`);
