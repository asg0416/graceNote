-- Phase 3 current season editability:
-- Applied seasons are normally historical records. The currently active
-- applied season is editable so admins can correct the current live grouping
-- and keep the season plan rows in sync.
...
stable
as $$
  select p_effective_week_date <= current_date
     and (p_end_week_date is null or (p_end_week_date + 6) >= current_date);
$$;
...
grant execute on function public.phase3_regrouping_season_is_current(date, date) to authenticated, service_role;
