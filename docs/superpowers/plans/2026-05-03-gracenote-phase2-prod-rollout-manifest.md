...
| P2-071 | 출석률 분모가 제출 여부와 현재 roster 보충에 따라 흔들림 | 사람 중심 구조에서는 주차 기준 active person이 분모여야 한다. 단, 과거 Phase 2 starts_at이 실제 과거 소속을 완전히 복원하지 못하므로 attendance snapshot도 그 주 active evidence로 봐야 함 | `admin-web/src/lib/attendanceMetrics.ts` 추가. denominator = week-date active distinct people + attendance snapshot people, numerator = present/late distinct people, no-meeting denominator excluded. `attendance/page.tsx` 카드/트렌드/인사이트/export에 적용 | `node --test src/lib/attendanceMetrics.test.ts`: 5 passed. `npx tsc --noEmit`: attendance/metrics no errors, existing churches error remains. consistency mismatch 0 | Admin attendance dashboard 품질 보정 운영 후보 |
...
| 2026-05-08 Admin attendance dashboard metric cleanup | 출석률 기준을 `week-date active distinct people + attendance snapshot people` 분모와 `present/late distinct people` 분자로 통일. no-meeting day 제외, `선택 주차 출석률` 라벨, spouse/family-aware sort 적용. `node --test src/lib/attendanceMetrics.test.ts`: 5 passed. consistency gate all mismatches 0 |
