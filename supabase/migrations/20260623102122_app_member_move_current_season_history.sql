-- Keep current-season edit history in sync when Flutter admin moves a member.
-- The live memberships remain the source of visibility; this only annotates
-- the current applied season plan so admin-web can show "season change history".
...
begin
  ...
  v_assignment_role := coalesce(nullif(v_member.role_in_group, ''), 'member');
  ...
  where s.church_id = v_member.church_id
    ...
    and s.effective_week_date <= v_effective_week
    and (s.end_week_date is null or s.end_week_date >= v_effective_week)
  ...
  if v_current_season.id is not null
    ...
    if v_target_plan_group_id is not null then
      ...
      elsif v_source_assignment_id is not null then
        update public.regrouping_plan_assignments
