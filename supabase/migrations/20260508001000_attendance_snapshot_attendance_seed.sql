-- Include submitted legacy attendance rows when generating roster snapshots.
-- This preserves historical weeks where membership starts_at/ends_at cannot
-- fully reconstruct the roster, but attendance was entered after the fact.
...
begin
  ...
  select distinct on (coalesce(md.person_id, mp.person_id))
    v_snapshot_id,
    coalesce(md.person_id, mp.person_id) as person_id,
    m.id as membership_id,
    a.directory_member_id,
    coalesce(a.group_id, g.id),
  ...
  left join public.people p on p.id = coalesce(md.person_id, mp.person_id)
  left join public.memberships m
    on m.person_id = coalesce(md.person_id, mp.person_id)
   and m.department_id = p_department_id
   and m.church_id = v_church_id
   and (
     m.legacy_member_directory_id = a.directory_member_id
     or m.group_id = coalesce(a.group_id, g.id)
   )
  where a.week_id = p_week_id
    and g.department_id = p_department_id
    and coalesce(md.person_id, mp.person_id) is not null
  order by
    coalesce(md.person_id, mp.person_id),
