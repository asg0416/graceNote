-- Keep default plan periods in sync when only the season shell period changes.
-- Rows with a custom period are intentionally left untouched.
...
begin
  ...
  -- Some dev/prod data can already be in a partially-updated state from older
  -- season period saves: the season shell may have changed while default group
  -- periods still keep the previous full-season range. Treat the dominant active
  -- group range as the old default range only when it is clearly the majority.
