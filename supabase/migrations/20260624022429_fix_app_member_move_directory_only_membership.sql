-- Fix app/admin group moves for directory-only members.
-- New-family members can have memberships keyed only by legacy_member_directory_id.
-- Reuse that row instead of inserting a second directory-only membership.
