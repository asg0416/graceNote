...
| 2026-05-08 Admin attendance dashboard metric cleanup | 출석률 기준을 `week-date active distinct people + attendance snapshot people + retroactive current roster backfill` 분모와 `present/late distinct people` 분자로 통일. no-meeting day 제외, `선택 주차 출석률` 라벨, 대상 산정 설명 패널, spouse/family-aware sort 적용. `node --test src/lib/attendanceMetrics.test.ts`: 6 passed. consistency gate all mismatches 0 |
