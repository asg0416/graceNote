-- Phase 3 member write RPC fix:
-- Preserve the legacy SmartBatch upsert behavior by resolving an existing
-- member_directory row before inserting a compatibility record.
...
as $$
declare
  v_member_directory_id uuid := p_member_directory_id;
...
begin
  if v_member_directory_id is not null then
    ...
    if not found then
      raise exception 'Member directory row % does not exist.', v_member_directory_id;
    end if;
  end if;
  ...
  if v_member_directory_id is null then
    select *
    into v_existing
    from public.member_directory
    where church_id = v_church_id
      ...
      and coalesce(public.phase2_normalize_phone(phone), '') = coalesce(v_phone, '')
      and coalesce(nullif(btrim(group_name), ''), '') = coalesce(v_group_name, '')
    order by
    ...
    if found then
      v_member_directory_id := v_existing.id;
    end if;
  end if;
