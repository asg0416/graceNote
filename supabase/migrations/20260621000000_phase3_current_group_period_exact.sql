-- Current-season edits must sync every plan group period into live groups.
-- The app reads groups.active_from/ended_at for prayer and attendance week
-- availability, so saving only archived groups leaves newly created active
-- groups stuck on their original period.
...
begin
  ...
  loop
    ...
    if v_ends_week is not null and v_ends_week < v_starts_week then
    ...
    set active_from = v_starts_week,
        ended_at = case
          when v_ends_week is null then null
          else v_ends_week::timestamp with time zone + interval '7 days' - interval '1 second'
        end,
        is_active = case
          when v_status = 'ended' then false
          when v_ends_week is not null
               and v_ends_week < public.phase3_week_start(current_date) then false
          else true
        end
