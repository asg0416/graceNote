...
const auditSql = "supabase/preprod_identity_link_audit_2026-05-22.sql";
const repairSql = "supabase/preprod_identity_link_auto_repair_2026-05-22.sql";
...
if (!shouldApply) {
  ...
  console.error("Expected usage: node scripts/preprod-identity-repair-psql.mjs --db-url-file <path> --apply-auto-repair");
  process.exit(2);
}
...
if (!allowKnownProd && knownProdProjectRefs.some((projectRef) => rawDbUrl.includes(projectRef))) {
  console.error("Refusing to run identity repair against the known production project ref.");
...
runSql("before identity audit", auditSql);
runSql("apply deterministic identity repair", repairSql);
runSql("after identity audit", auditSql);
