...
export default function DepartmentsPage() {
    ...
    const handleDeleteDept = (e: React.MouseEvent, id: string, name: string) => {
        e.preventDefault();
        e.stopPropagation();
        ...
        <div className="space-y-6 sm:space-y-8 max-w-7xl mx-auto px-1">
            <header className="flex flex-col lg:flex-row lg:items-end justify-between gap-6">
                ...
                {(isMaster || !assignedDeptId) && (
                    <Link
                        href={`/departments/new/settings${currentChurchId ? `?churchId=${currentChurchId}` : ''}`}
                        className="w-full lg:w-fit bg-indigo-600 dark:bg-indigo-500 text-white px-6 sm:px-8 py-3.5 sm:py-4 rounded-xl sm:rounded-2xl font-black text-xs sm:text-sm hover:bg-indigo-500 dark:hover:bg-indigo-400 hover:scale-[1.02] transition-all flex items-center justify-center gap-2.5 shadow-xl shadow-indigo-600/10 active:scale-95 border border-indigo-400/20 disabled:opacity-50"
                    >
            ...
            {loading ? (
                ...
                <div className="p-20 sm:p-32 flex flex-col items-center justify-center gap-6 text-center bg-white dark:bg-[#111827]/60 rounded-3xl border border-slate-200 dark:border-slate-800/80">
                ...
                <div className="grid grid-cols-1 gap-6 sm:gap-8">
                    {departments.map((dept) => (
                        <div key={dept.id} className="bg-white dark:bg-[#111827]/60 border border-slate-200 dark:border-slate-800/80 rounded-2xl sm:rounded-3xl overflow-hidden group hover:border-indigo-300 dark:hover:border-slate-700 transition-all duration-300 shadow-lg dark:shadow-none">
                            <div className="p-5 sm:p-8 border-b border-slate-100 dark:border-slate-800/60 bg-slate-50/50 dark:bg-slate-900/40 flex items-center justify-between">
                                ...
                                <div className="flex gap-2 sm:gap-3">
                                    ...
                                    {(isMaster || !assignedDeptId) && (
                                        <button
                                            onClick={(e) => handleDeleteDept(e, dept.id, dept.name)}
            ...
            >
                <form onSubmit={handleGroupSubmit} className="space-y-8 py-2">
                    <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-100 dark:border-slate-800/60 p-7 sm:p-8 shadow-sm space-y-8">
                ...
                <div className="space-y-6 py-2">
                    ...
                    <div className="space-y-2.5">
                        <p className="text-sm text-slate-600 dark:text-slate-400 font-bold">
                            확인을 위해 <span className="font-black text-slate-900 dark:text-white">{deleteTarget?.name}</span>을(를) 입력해주세요.
                        </p>
                        <input
                            type="text"
